...
public class LinearDrive extends DriveControlMethod {
    ...
    public static class LinearDriveCommand {
        ...
        public LinearDriveCommand(Pose2d goalPose, ChassisSpeeds goalEndSpeedFieldRelative, LinearDriveProfileConfig profileConfig) {
        ...
        public static LinearDriveProfileConfig fromJSON() {
            return new LinearDriveProfileConfig(
                new TrapezoidProfile.Constraints(
                    ...
                    JsonConstants.driveConstants.linearDriveProfileMaxLinearAcceleration.in(MetersPerSecondPerSecond)
                ),
                new TrapezoidProfile.Constraints(
                    JsonConstants.driveConstants.linearDriveProfileMaxAngularVelocity.in(RadiansPerSecond),
                    JsonConstants.driveConstants.linearDriveProfileMaxAngularAcceleration.in(RadiansPerSecondPerSecond)
                )
            );
        }

        public LinearDriveProfileConfig(TrapezoidProfile.Constraints linearConstraints, TrapezoidProfile.Constraints angularConstraints) {
    ...
    public LinearDrive(Drive drive) {
        super(drive, false);
        this.command = null;
        this.linearPath = new AdjustableLinearPath(new TrapezoidProfile.Constraints(0, 0), new TrapezoidProfile.Constraints(0, 0));
    }

    /**
     * Set the command for this control method. This should be called before activating the control method.
     * Set to null to stop the robot from moving.
    ...
    public void setCommand(LinearDriveCommand command) {
        ...
        if (command != null) {
            this.linearPath.setConstraints(command.profileConfig.linearConstraints, command.profileConfig.angularConstraints);
        }
    }
    ...
    public boolean isFinished() {
        ...
        if (!currentDistanceToTarget.isNear(Meters.zero(), JsonConstants.driveConstants.linearDriveMaxPositionError)) {
            return false;
        }

        var currentAngularError = Radians.of(drive.getPose().getRotation().minus(command.goalState.pose.getRotation()).getRadians());
        if (!currentAngularError.isNear(Radians.zero(), JsonConstants.driveConstants.linearDriveMaxAngularError)) {
            return false;
        }

        var driveChassisSpeeds = drive.getChassisSpeeds();

        var currentLinearVelocity = MetersPerSecond.of(
            Math.hypot(
                driveChassisSpeeds.vxMetersPerSecond,
                driveChassisSpeeds.vyMetersPerSecond));
        // Because the goal speeds are field-relative we have to convert them to robot-relative before comparing them to the current robot-relative speeds
        var goalLinearVelocity = getChassisLinearVelocity(command.goalState.speeds, command.goalState.pose.getRotation());
        if (!currentLinearVelocity.isNear(goalLinearVelocity, JsonConstants.driveConstants.linearDriveMaxLinearVelocityError)) {
            return false;
        }
        ...
        if (!currentAngularVelocity.isNear(goalAngularVelocity, JsonConstants.driveConstants.linearDriveMaxAngularVelocityError)) {
            return false;
        }
    ...
    private static LinearVelocity getChassisLinearVelocity(ChassisSpeeds speeds, Rotation2d heading) {
        ...
        return MetersPerSecond.of(speeds.vxMetersPerSecond * heading.getCos() + speeds.vyMetersPerSecond * heading.getSin());
    }
