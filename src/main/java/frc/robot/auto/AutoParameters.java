...
public class AutoParameters {
    ...
    if (parameters.containsKey(name)) {
      ...
      if (existingParameter == null) {
        ...
        throw new IllegalArgumentException(
            "Parameter with name " + name + " already exists with a different type");
  ...
  /**
   * Gets the parameter with the given name. Returns null if no such parameter exists. Caller is
   * expected to know the type of the parameter they are getting, and to cast it to the appropriate
   * type. For example, if you know that the parameter with name "speed" is a NumberParameter, you
   * would call getParameter("speed") and cast the result to NumberParameter or get the Value and
   * cast it to a double. If you try to cast it to the wrong type, you will get a ClassCastException
   * at runtime. Caller is expected to cast the result to the appropriate type, and should check for
  ...
  public abstract static class AutoParameter<T> {
  ...
  public static class ChoiceParameter<T> extends AutoParameter<T> {
    ...
    public boolean equals(AutoParameter<?> other) {
      ...
      // we know that other is a ChoiceParameter because of the super.equals check, so we can safely
      // cast it
  ...
  public static class NumberParameter extends AutoParameter<Double> {
    ...
    public boolean equals(AutoParameter<?> other) {
      if (!super.equals(other)) return false;
      // we know that other is a NumberParameter because of the super.equals check, so we can safely
      // cast it
