...
/**
 * The MatchState class handles tracking the state of the match and indicating when we can score vs. not score.
 */
public class MatchState {
    ...
    public enum ActiveHub {
        ...
        /**
         * Checks whether the hub is currently active, given information about who won auto and which alliance we are on
    ...
    public void enabledPeriodic(boolean manualRedOverridePressed, boolean manualBlueOverridePressed) {
        ...
        double shiftStartGracePeriodSeconds = JsonConstants.strategyConstants.shiftStartGracePeriod.in(Seconds);
        MatchShift shiftWithStartGrace = getCurrentShift(matchTime + shiftStartGracePeriodSeconds, lastMatchTime + shiftStartGracePeriodSeconds);

        Logger.recordOutput("MatchState/startGraceShift", shiftWithStartGrace);

        double shiftEndGracePeriodSeconds = JsonConstants.strategyConstants.shiftEndGracePeriod.in(Seconds);
        MatchShift shiftWithEndGrace = getCurrentShift(matchTime - shiftEndGracePeriodSeconds, lastMatchTime - shiftEndGracePeriodSeconds);

        Logger.recordOutput("MatchState/endGraceShift", shiftWithEndGrace);

        lastMatchTime = matchTime;

        this.canScore = currentShift.getActiveHub().isHubActive(wonAuto, hasDeterminedAutoWinner) || shiftWithStartGrace.getActiveHub().isHubActive(wonAuto, hasDeterminedAutoWinner) || shiftWithEndGrace.getActiveHub().isHubActive(wonAuto, hasDeterminedAutoWinner);
    }
    ...
    public MatchShift getCurrentShift(double currentMatchTime, double lastMatchTime) {
        // The behavior of this method is determined largely by the docs here https://github.wpilib.org/allwpilib/docs/release/java/edu/wpi/first/wpilibj/DriverStation.html#getMatchTime()
        if (DriverStation.isFMSAttached() || currentMatchTime <= lastMatchTime) {
            Logger.recordOutput("MatchState/isInMatch", true);
            // Either FMS is attached or time is counting down. Either way, time is decreasing so we are in a "match" situation
        ...
        if (currentMatchTime > lastMatchTime) {
            // If match time is counting up, we are not in practice mode and therefore we should be allowed to shoot always
     ...
     * 
     * @return {@code true} if the hub is currently active, will soon be active, or has just become inactive, {@code false} otherwise
     */
